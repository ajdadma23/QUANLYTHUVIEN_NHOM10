...
public class KhachHangData {
    ...
    public static ArrayList<KhachHang> getListkh() {
        ...
        try {
            ...
            KhachHang kh;
            while (rs.next()) {
                kh = new KhachHang(rs.getString("Ma_Khach_hang"), rs.getString("Ten_Khach_hang"), rs.getDate("Ngay_sinh"), rs.getString("Dia_chi"), rs.getString("So_dien_thoai"));
                ListKH.add(kh);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static ArrayList<KhachHang> addKH(String maKhachHang) {
        ...
        try {
            ps = (PreparedStatement) Connect.getConnect().prepareStatement("INSERT INTO `khach_hang`"
                    + "(`Ma_Khach_hang`, `Ten_Khach_hang`, `Ngay_sinh`, `Dia_chi`, `So_dien_thoai`)"
    ...
    public static ArrayList<KhachHang> updateKH(String maKhachHang) {
        ...
        try {
            ps = (PreparedStatement) Connect.getConnect().prepareStatement("UPDATE `khach_hang` SET `Ten_Khach_hang`='" + JFrameKhachHang.txt2.getText() + "',`Ma_Khach_hang`='" + JFrameKhachHang.txt1.getText() + "',`Ngay_sinh`='" + Date.valueOf(JFrameKhachHang.txt3.getText()) + "',`Dia_chi`='" + JFrameKhachHang.txt4.getText() + "',`So_dien_thoai`='" + JFrameKhachHang.txt5.getText() + "' WHERE Ma_Khach_hang = '" + maKhachHang+ "'");
            ps.executeUpdate();
